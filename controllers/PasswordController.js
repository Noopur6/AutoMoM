...
module.exports.forgot = (req, res) => {
    ...
    User.findOneAndUpdate({email : req.body.email}, {$set:{resetToken:encryptedToken}}, { returnNewDocument: true }, function(err, user){
        if(err) {
            res.send({
                status:"E",
                error: [
            ...
            //send mail
            commonUtils.sendMail(req.body.email, null, 'AutoMoM: Reset Password', emailTemplate.bringThatForgotPasswordTemplate(url, req.body.email));
        }
    }); 
}